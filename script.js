...
// Fixed Profile Card Integration - Add this to your script.js or as a separate file

// Enhanced QuizController that properly updates the profile card
class EnhancedQuizController {
    constructor() {
        this.currentQuestion = 1;
        this.totalQuestions = 6;
        this.answers = {};
        this.journeyController = window.journeyController;
        this.init();
    }
    ...
    setupEventListeners() {
        ...
        // Slider updates with profile card integration
        document.getElementById('age').addEventListener('input', (e) => {
            document.getElementById('ageValue').textContent = e.target.value + ' years';
            this.answers.age = parseInt(e.target.value);
        });

        document.getElementById('sleepHours').addEventListener('input', (e) => {
            document.getElementById('sleepHoursValue').textContent = e.target.value + ' hours';
            this.answers.sleepHours = parseFloat(e.target.value);
            // Update profile card immediately
            if (this.journeyController) {
                this.journeyController.updateProfileData('duration', e.target.value);
            }
        });

        document.getElementById('sleepLatency').addEventListener('input', (e) => {
            document.getElementById('sleepLatencyValue').textContent = e.target.value + ' minutes';
            this.answers.sleepLatency = parseInt(e.target.value);
            // Update profile card immediately
            if (this.journeyController) {
                this.journeyController.updateProfileData('latency', e.target.value);
            }
        });

        document.getElementById('nightAwakenings').addEventListener('input', (e) => {
            document.getElementById('nightAwakeningsValue').textContent = e.target.value + ' times';
            this.answers.nightAwakenings = parseInt(e.target.value);
        });

        // Radio button listeners for stress and chronotype
        document.querySelectorAll('input[name="stress"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                if (e.target.checked) {
                    this.answers.stressLevel = parseInt(e.target.value);
                    const stressLabels = ['Very Low', 'Low', 'Moderate', 'High', 'Very High'];
                    if (this.journeyController) {
                        this.journeyController.updateProfileData('stress', stressLabels[this.answers.stressLevel - 1]);
                    }
                }
            });
        });
        ...
        document.querySelectorAll('input[name="chronotype"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                if (e.target.checked) {
                    this.answers.chronotype = parseInt(e.target.value);
                    if (this.journeyController) {
                        this.journeyController.updateProfileData('chronotype', this.answers.chronotype);
                    }
                }
    ...
    saveCurrentAnswer() {
        switch (this.currentQuestion) {
            ...
            case 3:
                const stress = document.querySelector('input[name="stress"]:checked');
                if (stress) {
                    this.answers.stressLevel = parseInt(stress.value);
                    const stressLabels = ['Very Low', 'Low', 'Moderate', 'High', 'Very High'];
                    if (this.journeyController) {
                        this.journeyController.updateProfileData('stress', stressLabels[this.answers.stressLevel - 1]);
                    }
            ...
            case 6:
                const chronotype = document.querySelector('input[name="chronotype"]:checked');
                if (chronotype) {
                    this.answers.chronotype = parseInt(chronotype.value);
                    if (this.journeyController) {
                        this.journeyController.updateProfileData('chronotype', this.answers.chronotype);
                    }
                }
    ...
    submitQuiz() {
        ...
        setTimeout(() => {
            const match = findBestMatch(this.answers);
            this.displayResults(this.answers, match);
        }, 2000);
    }
    ...
    displayResults(userProfile, match) {
        ...
        // Update profile card with twin info
        if (this.journeyController) {
            this.journeyController.updateProfileData('twin', `Participant ${twinId}`);
        }
        
        // Highlight the matched participant
...
// Enhanced Dashboard Integration - Updates profile card from dashboard sliders
function enhanceDashboardIntegration() {
    const journeyController = window.journeyController;
    if (!journeyController) return;
    ...
    if (durationSlider) {
        durationSlider.addEventListener("input", function() {
            document.getElementById("duration-value").textContent = this.value + "h";
            journeyController.updateProfileData('duration', this.value);
        });
    }
    ...
    if (stressSlider) {
        stressSlider.addEventListener("input", function() {
            ...
            // Convert numeric stress to label
            let stressLabel = 'Moderate';
            if (this.value < 20) stressLabel = 'Low';
            else if (this.value < 35) stressLabel = 'Moderate';
            else if (this.value < 50) stressLabel = 'High';
            else stressLabel = 'Very High';
            
            journeyController.updateProfileData('stress', stressLabel);
        });
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        // Also pre-populate profile card with any existing dashboard values
        const journeyController = window.journeyController;
        if (journeyController) {
            ...
            if (durationSlider) {
                journeyController.updateProfileData('duration', durationSlider.value);
            }
            
            if (stressSlider) {
                let stressLabel = 'Moderate';
                const stressValue = stressSlider.value;
                if (stressValue < 20) stressLabel = 'Low';
                else if (stressValue < 35) stressLabel = 'Moderate';
                else if (stressValue < 50) stressLabel = 'High';
                else stressLabel = 'Very High';
...
// Debug function to manually test profile card updates
function testProfileCard() {
    const controller = window.journeyController;
    if (controller) {
        controller.updateProfileData('duration', '7.5');
        controller.updateProfileData('stress', 'Moderate');
        controller.updateProfileData('latency', '15');
        controller.updateProfileData('chronotype', 3);
        controller.updateProfileData('twin', 'Participant 5');
